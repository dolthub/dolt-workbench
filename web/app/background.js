...
const userDataPath = electron__WEBPACK_IMPORTED_MODULE_12__.app.getPath("userData");
const schemaPath = path__WEBPACK_IMPORTED_MODULE_11___default().join(userDataPath, "schema.gql");
...
function createGraphqlSeverProcess() {
  const serverPath =  false ? 0 : path__WEBPACK_IMPORTED_MODULE_11___default().join("../graphql-server", "dist", "main.js");
  serverProcess = electron__WEBPACK_IMPORTED_MODULE_12__.utilityProcess.fork(serverPath, [], {
    stdio: "pipe"
  });
  serverProcess?.stdout?.on("data", chunk => {
    ...
    mainWindow?.webContents.executeJavaScript(`
        console.info('Server Log:', ${_babel_runtime_corejs3_core_js_stable_json_stringify__WEBPACK_IMPORTED_MODULE_9___default()(chunk.toString("utf8"))})`);
  });
  serverProcess?.stderr?.on("data", chunk => {
    ...
    mainWindow?.webContents.executeJavaScript(`
        console.error('Server Log:', ${_babel_runtime_corejs3_core_js_stable_json_stringify__WEBPACK_IMPORTED_MODULE_9___default()(chunk.toString("utf8"))})`);
  });
}
electron__WEBPACK_IMPORTED_MODULE_12__.app.on("ready", () => {
  ...
  if (isProd) {
    _babel_runtime_corejs3_core_js_stable_set_timeout__WEBPACK_IMPORTED_MODULE_10___default()(async () => {
...
electron__WEBPACK_IMPORTED_MODULE_12__.app.on("before-quit", () => {
  if (serverProcess) {
    serverProcess.kill();
    serverProcess = null;
  }
...
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle("get-headers", (event, arg) => {
  const headers = _objectSpread({}, arg);
  return headers;
});
electron__WEBPACK_IMPORTED_MODULE_12__.ipcMain.handle("api-config", async () => {
  const cfg = {
