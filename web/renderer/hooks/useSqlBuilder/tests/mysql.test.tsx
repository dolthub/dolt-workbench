  ...
  const tests = [
    {
      ...
      name: "DOLT_MERGE",
      args: [],
      expected: "CALL DOLT_MERGE();",
    },
    {
      desc: "with one arg",
      name: "DOLT_BRANCH",
      args: ["branch"],
      expected: "CALL DOLT_BRANCH('branch');",
    },
    {
      desc: "with two args",
      name: "DOLT_CHECKOUT",
      args: ["-b", "newbranch"],
      expected: "CALL DOLT_CHECKOUT('-b', 'newbranch');",
    },
    {
      desc: "with escaped single quote",
      name: "DOLT_COMMIT",
      args: ["-m", "single quote's escape"],
      expected: "CALL DOLT_COMMIT('-m', 'single quote\\'s escape');",
    },
  ];

  tests.forEach(test => {
    it(test.desc, async () => {
      const { getCallProcedure } = await renderUseSqlBuilder();
